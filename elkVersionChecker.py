...
Monitors the Elasticsearch download page for new versions and sends
...
Configuration:
    ...
    - VERSION_FILE (default: elasticsearch_last_version.txt)
...
Usage:
    python elasticsearch_version_checker.py [--enable-email] [--enable-slack] [--enable-teams] [--dry-run] [--verbose]
"""
...
class Config:
    ...
    def __init__(self, args):
        ...
        # File configuration
        self.version_file = args.version_file or os.getenv('VERSION_FILE', 'elasticsearch_last_version.txt')
...
def get_latest_version(retry_count: int = 0) -> Optional[str]:
    """
    Fetch the latest Elasticsearch version from the download page
    ...
    try:
        ...
        response = requests.get(
            ...
            headers={'User-Agent': 'Elasticsearch-Version-Checker/2.0'}
        )
        response.raise_for_status()
        
        # Search for version pattern in HTML
        match = re.search(VERSION_PATTERN, response.text, re.IGNORECASE)
        if not match:
            logger.error("Version pattern not found in page content")
...
def send_email_notification(config: Config, version: str) -> bool:
    ...
    msg['Subject'] = f"New Elasticsearch Version Detected: {version}"
    
    body = f"""A new version of Elasticsearch has been released.

Version: {version}
Download Page: {URL}

This is an automated notification from the Elasticsearch Version Checker.
"""
...
def send_slack_notification(config: Config, version: str) -> bool:
    ...
    payload = {
        "text": f":tada: New Elasticsearch version available: *{version}*\nCheck it out here: {URL}"
    }
...
def send_teams_notification(config: Config, version: str) -> bool:
    ...
    if is_power_automate:
        ...
        payload = {
            "title": "New Elasticsearch Version Available",
            "version": version,
            "status": "Stable Release",
            "description": f"A new version of Elasticsearch has been released: {version}",
            "url": URL,
            "emoji": "🔍"
        }
                        ...
                        "body": [
                            {
                                ...
                                "size": "Large",
                                "weight": "Bolder",
                                "text": "🔍 New Elasticsearch Version Available",
                                "wrap": True,
                                "color": "Good"
                            },
                            {
                            ...
                            {
                                "type": "TextBlock",
                                "text": "A new version of Elasticsearch has been released and is ready for download.",
                                "wrap": True
                            }
                        ],
                        "actions": [
...
def notify_new_version(config: Config, version: str) -> bool:
    ...
    """
    logger.info(f"New Elasticsearch version detected: {version}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Monitor Elasticsearch for new versions',
...
Examples:
  ...
  %(prog)s --enable-email --enable-teams
    ...
    parser.add_argument(
        '--version-file',
        help='Path to version tracking file (default: elasticsearch_last_version.txt)'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
