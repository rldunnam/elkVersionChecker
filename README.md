...
A Python script that monitors the Elasticsearch download page for new versions and sends automated notifications via email and/or Slack when updates are detected.

## Features

- 🔍 Automatically checks for new Elasticsearch versions
...
- 🎯 Monitors official Elastic.co download page
...
1. **Clone or download the script**
   ...
   cd elasticsearch-version-checker
   ```
...
| `VERSION_FILE` | Path to version tracking file | elasticsearch_last_version.txt | No |
...
EMAIL_FROM=elasticsearch-alerts@yourcompany.com
...
# Optional Settings
VERSION_FILE=elasticsearch_last_version.txt
DRY_RUN=false
```
...
```bash
python elasticsearch_version_checker.py
```

**Enable email notifications:**
```bash
python elasticsearch_version_checker.py --enable-email
```

**Enable Slack notifications:**
```bash
python elasticsearch_version_checker.py --enable-slack
```

**Enable both:**
```bash
python elasticsearch_version_checker.py --enable-email --enable-slack
```
...
```bash
python elasticsearch_version_checker.py --dry-run --enable-email --enable-slack
```

**Verbose logging:**
```bash
python elasticsearch_version_checker.py --verbose --enable-email
```

**Test with dry-run and verbose:**
```bash
python elasticsearch_version_checker.py --dry-run --verbose --enable-email --enable-slack
```

### Get Help

```bash
python elasticsearch_version_checker.py --help
```
...
0 */6 * * * cd /path/to/script && /usr/bin/python3 elasticsearch_version_checker.py >> /var/log/elasticsearch_checker.log 2>&1
```
...
# Add the cron job
0 */6 * * * /usr/bin/python3 /path/to/elasticsearch_version_checker.py >> /var/log/elasticsearch_checker.log 2>&1
```
...
cat > ~/.elasticsearch_credentials << 'EOF'
...
# Secure it
chmod 600 ~/.elasticsearch_credentials

# In crontab
0 */6 * * * . ~/.elasticsearch_credentials && /usr/bin/python3 /path/to/elasticsearch_version_checker.py >> /var/log/elasticsearch_checker.log 2>&1
```
...
1. **Fetches** the Elasticsearch download page (https://www.elastic.co/downloads/elasticsearch)
2. **Parses** the HTML using regex to find the version number
3. **Extracts** the latest version from the page content
4. **Compares** with the last known version stored in a local file
5. **Notifies** via configured channels (email/Slack) if a new version is detected
...
chmod 755 elasticsearch_version_checker.py
```
...
# Version tracking
elasticsearch_last_version.txt
...
**Problem:** `Version pattern not found in page content`

**Solutions:**
- Check if the Elasticsearch download page is accessible
- Verify internet connectivity
- Check if page structure changed (Elastic may have updated their site)
- Try with `--verbose` flag to see what content is being fetched
- The regex pattern may need updating if Elastic changes their page format

### Version pattern changed

If Elastic.co changes their page structure, you may need to update the regex pattern in the script. The current pattern is:
```python
...
```bash
# Redirect to file
python elasticsearch_version_checker.py --enable-email >> elasticsearch.log 2>&1

# View logs in real-time
tail -f elasticsearch.log

# With cron, specify log file in crontab
0 */6 * * * /usr/bin/python3 /path/to/script.py >> /var/log/elasticsearch_checker.log 2>&1
```
...
result = subprocess.run(
    ['python3', 'elasticsearch_version_checker.py'],
    env=env,
    capture_output=True
)
...
```bash
python elasticsearch_version_checker.py \
  --version-file /custom/path/elasticsearch_version.txt \
  --enable-email
```
...
```bash
# Infrastructure team
EMAIL_TO=infra-team@company.com VERSION_FILE=elasticsearch_version_infra.txt \
python elasticsearch_version_checker.py --enable-email

# Security team
EMAIL_TO=security-team@company.com VERSION_FILE=elasticsearch_version_security.txt \
python elasticsearch_version_checker.py --enable-email
```

### Integration with CI/CD

Check for new Elasticsearch versions in your pipeline:
...
check-elasticsearch-version:
  script:
    ...
    - python elasticsearch_version_checker.py --enable-slack --slack-webhook $SLACK_WEBHOOK
  only:
    - schedules
```
...
### Understanding Elasticsearch Versioning

Elasticsearch uses semantic versioning: `MAJOR.MINOR.PATCH`
- Example: `8.11.3`
- **Major versions** (8.x.x) introduce breaking changes
- **Minor versions** (x.11.x) add features in a backward-compatible manner
- **Patch versions** (x.x.3) contain bug fixes and security updates

### Release Cycles

- Elasticsearch typically releases new minor versions every few months
- Patch releases occur more frequently for critical fixes
- Major version upgrades require careful planning due to breaking changes

### Why Monitor Elasticsearch Versions?

- **Security patches** - Critical vulnerabilities are fixed in new releases
- **Performance improvements** - Each release often includes optimizations
- **New features** - Stay current with latest capabilities
- **Bug fixes** - Resolve known issues affecting your deployment
- **Compatibility** - Plan upgrades with your Elastic Stack (Kibana, Logstash, Beats)
- **Support lifecycle** - Older versions eventually lose support

### The Elastic Stack

This checker monitors Elasticsearch specifically. Remember that the Elastic Stack includes:
- **Elasticsearch** - Search and analytics engine (what this script monitors)
- **Kibana** - Visualization and management interface
- **Logstash** - Data processing pipeline
- **Beats** - Lightweight data shippers

**Important:** When upgrading Elasticsearch, ensure compatibility with your other Elastic Stack components. Generally, upgrade Elasticsearch first, then the other components.

## Version Compatibility Notes

- **Upgrade order:** Elasticsearch → Kibana → Logstash → Beats
- **Cross-version compatibility:** Typically N-1 (e.g., Kibana 8.11 works with Elasticsearch 8.10-8.11)
- **Breaking changes:** Major version upgrades (7.x → 8.x) require migration planning
- **Testing:** Always test version upgrades in non-production environments first
...
- [NGINX Version Checker](../nginx-version-checker/)
- [Docker Version Checker](../docker-version-checker/)
- [Kibana Version Checker](../kibana-version-checker/)
...
- Changed default version file location from `/tmp/` to current directory
- Added comprehensive documentation

### Version 1.0
- Initial release
- Basic version checking from Elasticsearch download page
- Slack notifications only
- Simple console output
- Version file stored in `/tmp/`

## Acknowledgments

- Elastic team for maintaining clear version information on their download page
- The open-source community for Python libraries used in this project

## Additional Resources

- [Elasticsearch Official Documentation](https://www.elastic.co/guide/en/elasticsearch/reference/current/index.html)
- [Elastic Stack Upgrade Guide](https://www.elastic.co/guide/en/elastic-stack/current/upgrading-elastic-stack.html)
